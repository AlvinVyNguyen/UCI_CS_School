...
for i in irange(0,7):
    count = 1000*2**i
    alist.append((random.random()*1000* 2**i,random.random()*1000* 2**i))
    p = Performance(lambda: closest_2d(alist),lambda : random.random()*count,5,'closest_2d, size = '+str(count))
    p.evaluate()
    p.analyze()
    print()