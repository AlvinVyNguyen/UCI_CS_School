...
# Define add_after ITERATIVELY

def add_after(ll,value,new):
    pass


# Define duplicate RECURSIVELY

def add_after_r(ll,value,new):
    pass
...
def build_balanced_bst(l):
    if len(l) != 0:
        ...
        return TN(l[0]) 
    lenght_of_tree = len(l)
    trimmed_tree = lenght_of_tree - 1
    middle = trimmed_tree//2
    left_side = build_balanced_bst(l[:middle])
    right_side = build_balanced_bst(l[middle+1:])
    full_tree = TN(l[middle], left_side, right_side)
    return full_tree
...
class StringVar_WithHistory(StringVar):
    ...
    def undo (self):
        if len(self.list1) <= 1:
            ...
            self.list1.pop(-1)
            StringVar.set(self,self.list1[-1])
...
if __name__ == '__main__':
    print('Testing add_after')
    ll = list_to_ll([1,8,2,4,2,5,2])
    print('\noriginal list              = ',str_ll(ll))
    add_after(ll,2,-1)
    print('resulting list             = ',str_ll(ll))
